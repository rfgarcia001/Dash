...
function FunnelStep({ label, icon, value, tone, formatNumber }: { label: string; icon: React.ReactNode; value: number; tone: 'brand' | 'management' | 'neutral'; formatNumber: (val: number) => string }) {
  const toneClasses = tone === 'brand'
    ...
    : tone === 'management'
      ...
      : 'bg-[var(--surface-3)] border border-[var(--border-hairline)] text-zinc-100';
  return (
  ...
  return (
    <div className="flex flex-col items-center my-1 relative h-12 w-full max-w-xs">
      ...
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-[var(--surface-2)] border border-[var(--border-hairline)] rounded-full px-4 py-1.5 shadow-md whitespace-nowrap z-10 flex flex-col items-center">
        <span className="text-[length:var(--type-caption)] font-medium text-zinc-400 uppercase tracking-wider">{label}</span>
        <span className="text-xs font-mono font-bold text-[var(--brand-strategy)] tabular-nums">{formatPercent(value)}</span>
      </div>
    </div>
  );
}
      ...
      <Card className="flex flex-col items-center w-full min-h-[400px] p-6 sm:p-10">
        ...
        <div className="flex flex-col items-center gap-1 w-full max-w-md">
          ...
          <div className="w-full relative group">
            <button
              type="button"
              className="w-full bg-[var(--surface-3)] border border-[var(--border-hairline)] text-zinc-200 rounded-[var(--radius-panel)] py-2.5 flex flex-col items-center justify-center shadow-sm hover:border-[var(--brand-strategy)]/50 transition-colors focus-visible:border-[var(--brand-strategy)]"
              aria-describedby="funil-outras-vendas-tooltip"
            >
              <span className="text-[length:var(--type-caption)] font-bold uppercase tracking-widest mb-0.5 text-zinc-400">Vendas (Outras / Orgânicas)</span>
              <span className="font-mono font-bold text-3xl leading-none text-[var(--brand-strategy)] tabular-nums">{formatNumber(geral.vendasIngressos - geral.vendasTrafego)}</span>
            </button>

            <div
              id="funil-outras-vendas-tooltip"
              role="tooltip"
              className="absolute top-full left-1/2 -translate-x-1/2 mt-2 w-64 bg-[var(--surface-2)] border border-[var(--border-hairline)] text-white text-xs rounded-[var(--radius-panel)] shadow-[var(--elevation-2)] opacity-0 invisible group-hover:opacity-100 group-hover:visible group-focus-within:opacity-100 group-focus-within:visible transition-all z-50 p-3 pointer-events-none"
            >
              <div className="font-bold border-b border-[var(--border-hairline)] pb-2 mb-2 text-[var(--brand-strategy)]">Origens (Outras/Orgânicas)</div>
              <div className="flex flex-col gap-1 max-h-48 overflow-y-auto">
                {outrasVendas.map((s: any) => (
                  <div key={s.name} className="flex justify-between items-center">
                    <span className="truncate pr-2 font-medium text-zinc-300">{s.name === 'SEM ORIGEM' ? 'Desconhecida' : s.name}</span>
                    <span className="font-bold text-[var(--brand-strategy)] font-mono tabular-nums">{s.count}</span>
                  </div>
                ))}
                {outrasVendas.length === 0 && (
                  <div className="text-zinc-500 italic">Nenhuma venda encontrada</div>
                )}
              </div>
            </div>
          </div>
            ...
            <table className="w-full text-left text-sm whitespace-nowrap lg:whitespace-normal">
              <thead className="table-heading font-mono font-bold uppercase tracking-wider text-[11px]">
                <tr>
                  ...
                  <th className="px-2 py-3.5 text-center text-zinc-500">&rarr;</th>
                  <th className="px-3.5 py-3.5 text-center">Vz da Pag.<br/><span className="text-[10px] font-normal text-zinc-400">Visualizações</span></th>
                  <th className="px-2 py-3.5 text-center text-zinc-500">&rarr;</th>
                  <th className="px-3.5 py-3.5 text-center">IC<br/><span className="text-[10px] font-normal text-zinc-400">Initiate Checkout</span></th>
                  <th className="px-2 py-3.5 text-center text-zinc-500">&rarr;</th>
                  <th className="px-3.5 py-3.5 text-center">Livros Vendidos<br/><span className="text-[10px] font-normal text-zinc-400">Tráfego Pago</span></th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[var(--border-hairline)] text-zinc-300 font-mono text-xs">
                {sortedCampaigns.map((camp: any) => (
                  <React.Fragment key={camp.name}>
                    <tr className="hover:bg-white/[0.045] transition-colors group bg-[var(--surface-1)]">
                      <td className="px-3.5 py-3 font-sans font-bold text-zinc-100 flex items-center gap-2 max-w-[220px] xl:max-w-[320px]">
                        <button
                          ...
                          className="min-h-11 min-w-11 -my-2 inline-flex shrink-0 items-center justify-center rounded-[var(--radius-control)] text-[var(--text-subtle)] hover:bg-white/[0.06] hover:text-[var(--brand-strategy)]"
                        >
                      ...
                      <td className="px-3.5 py-3 text-center font-medium tabular-nums bg-white/[0.02]">{formatNumber(camp.impressoes)}</td>
                      <td className="px-2 py-3 text-center text-xs text-zinc-500 tabular-nums">{formatPercent(camp.ctr)}</td>
                      <td className="px-3.5 py-3 text-center font-medium tabular-nums bg-[var(--brand-strategy)]/10 text-[var(--brand-strategy)]">{formatNumber(camp.cliques)}</td>
                      <td className="px-2 py-3 text-center text-xs text-zinc-500 tabular-nums">{formatPercent(camp.cliques > 0 ? camp.landingPageViews / camp.cliques : 0)}</td>
                      ...
                      <td className="px-2 py-3 text-center text-xs text-zinc-500 tabular-nums">{formatPercent(camp.landingPageViews > 0 ? camp.initiateCheckout / camp.landingPageViews : 0)}</td>
                      ...
                      <td className="px-2 py-3 text-center text-xs text-zinc-500 tabular-nums">{formatPercent(camp.initiateCheckout > 0 ? camp.comprasTrafego / camp.initiateCheckout : 0)}</td>
                      <td className="px-3.5 py-3 text-center font-black tabular-nums bg-[var(--brand-strategy)]/20 text-[var(--brand-strategy)]">{formatNumber(camp.comprasTrafego)}</td>
                    </tr>
                    ...
                    {expandedCampaigns[camp.name] && camp.sets.map((set: any) => (
                      <tr key={`${camp.name}-${set.name}`} className="bg-black/10 hover:bg-white/[0.035] transition-colors">
                        ...
                        <td className="px-3.5 py-2.5 text-center text-[11px] text-zinc-400 tabular-nums">{formatNumber(set.impressoes)}</td>
                        <td className="px-2 py-2.5 text-center text-[10px] text-zinc-500 tabular-nums">{formatPercent(set.ctr)}</td>
                        <td className="px-3.5 py-2.5 text-center text-[11px] text-[var(--brand-strategy)] font-medium tabular-nums">{formatNumber(set.cliques)}</td>
                        <td className="px-2 py-2.5 text-center text-[10px] text-zinc-500 tabular-nums">{formatPercent(set.cliques > 0 ? set.landingPageViews / set.cliques : 0)}</td>
                        ...
                        <td className="px-2 py-2.5 text-center text-[10px] text-zinc-500 tabular-nums">{formatPercent(set.landingPageViews > 0 ? set.initiateCheckout / set.landingPageViews : 0)}</td>
                        ...
                        <td className="px-2 py-2.5 text-center text-[10px] text-zinc-500 tabular-nums">{formatPercent(set.initiateCheckout > 0 ? set.comprasTrafego / set.initiateCheckout : 0)}</td>
                        <td className="px-3.5 py-2.5 text-center text-[11px] text-[var(--brand-strategy)] font-black tabular-nums">{formatNumber(set.comprasTrafego)}</td>
                      </tr>
                    ))}
                  </React.Fragment>
                ))}
              ...
              {metricsData.campaigns.length > 0 && (
                <tfoot className="bg-white/[0.045] border-t border-[var(--border-hairline)] font-mono font-bold text-zinc-100">
                  <tr>
                    <td className="px-3.5 py-4 text-[11px] uppercase tracking-wider text-[var(--brand-strategy)]">Total do Período</td>
                    <td className="px-3.5 py-4 text-center tabular-nums">{formatNumber(campaignTotals.impressoes)}</td>
                    <td className="px-2 py-4 text-center text-zinc-500 tabular-nums">{formatPercent(campaignTotals.impressoes > 0 ? campaignTotals.cliques / campaignTotals.impressoes : 0)}</td>
                    <td className="px-3.5 py-4 text-center text-[var(--brand-strategy)] tabular-nums">{formatNumber(campaignTotals.cliques)}</td>
                    <td className="px-2 py-4 text-center text-zinc-500 tabular-nums">{formatPercent(campaignTotals.cliques > 0 ? campaignTotals.landingPageViews / campaignTotals.cliques : 0)}</td>
                    <td className="px-3.5 py-4 text-center text-emerald-300 tabular-nums">{formatNumber(campaignTotals.landingPageViews)}</td>
                    <td className="px-2 py-4 text-center text-zinc-500 tabular-nums">{formatPercent(campaignTotals.landingPageViews > 0 ? campaignTotals.initiateCheckout / campaignTotals.landingPageViews : 0)}</td>
                    ...
                    <td className="px-2 py-4 text-center text-zinc-500 tabular-nums">{formatPercent(campaignTotals.initiateCheckout > 0 ? campaignTotals.compras / campaignTotals.initiateCheckout : 0)}</td>
                    <td className="px-3.5 py-4 text-center text-[var(--brand-strategy)] font-black tabular-nums">{formatNumber(campaignTotals.compras)}</td>
                  </tr>
                </tfoot>
              )}
            </table>
